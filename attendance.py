...
class Attendance:
    ...
    def __init__(self,root):
        ...
        studentId_entry = ttk.Entry(left_frame,textvariable=self.va_id,width=15,font=("verdana",12,"bold"))
    ...
    # export upadte
    def action(self):
        if self.va_id.get()=="" or self.var_roll.get=="" or self.var_name.get()=="" or self.var_time.get()=="" or self.var_date.get()=="" or self.var_attend.get()=="Status":
            messagebox.showerror("Error","Please Fill All Fields are Required!",parent=self.root)
        else:
            try:
