...
class Student:
    ...
    # This part is image labels setting start
        # first header image
        img = Image.open(
            r"C:\Users\VEDIKA MANDRE\Desktop\Face recognition system\Images_GUI\banner.jpg")
        ...
        # backgorund image
        bg1 = Image.open(
            r"C:\Users\VEDIKA MANDRE\Desktop\Face recognition system\Images_GUI\bg3.jpg")
        ...
        dep_combo["values"] = ("Select Department", "Computer",
                               "IT", "Mechanical", "Electronics", "Civil", "Other")
        dep_combo.current(0)
        ...
        # label Year
        yea_label = Label(current_course_frame, text="Year", font=(
            "verdana", 12, "bold"), bg="white", fg="navyblue")
        yea_label.grid(row=0, column=2, padx=5, pady=15)

        # combo box
        yea_combo = ttk.Combobox(current_course_frame, textvariable=self.var_yea, width=15, font=(
            "verdana", 12, "bold"), state="readonly")
        yea_combo["values"] = ("Select Year", "FE", "SE", "TE", "BE")
        yea_combo.current(0)
        yea_combo.grid(row=0, column=3, padx=5, pady=15, sticky=W)

        # -------------------------------------------------------------

        # label batch
        batch_label = Label(current_course_frame, text="Batch", font=(
            "verdana", 12, "bold"), bg="white", fg="navyblue")
        batch_label.grid(row=1, column=0, padx=5, sticky=W)

        # combo box
        batch_combo = ttk.Combobox(current_course_frame, textvariable=self.var_batch, width=15, font=(
            "verdana", 12, "bold"), state="readonly")
        batch_combo["values"] = ("Select batch", "2017-21",
                                "2018-22", "2019-23", "2020-24", "2021-25")
        batch_combo.current(0)
        batch_combo.grid(row=1, column=1, padx=5, pady=15, sticky=W)
        ...
        studentId_entry = ttk.Entry(
            class_Student_frame, textvariable=self.va_std_id, width=15, font=("verdana", 12, "bold"))
        ...
        div_combo = ttk.Combobox(class_Student_frame, textvariable=self.var_div, width=13, font=(
            "verdana", 12, "bold"), state="readonly")
        div_combo["values"] = ("Select Div", "A", "B", "C", "Other")
        div_combo.current(0)
        ...
        # save button
        save_btn = Button(btn_frame, text="Save", command=self.add_data, width=7, font=(
            "verdana", 12, "bold"), fg="white", bg="navyblue")
        save_btn.grid(row=0, column=0, padx=5, pady=10, sticky=W)

        # update button
        update_btn = Button(btn_frame, text="Update", command=self.update_data, width=7, font=(
            "verdana", 12, "bold"), fg="white", bg="navyblue")
        update_btn.grid(row=0, column=1, padx=5, pady=8, sticky=W)

        # delete button
        del_btn = Button(btn_frame, text="Delete", command=self.delete_data, width=7, font=(
            "verdana", 12, "bold"), fg="white", bg="navyblue")
        del_btn.grid(row=0, column=2, padx=5, pady=10, sticky=W)

        # reset button
        reset_btn = Button(btn_frame, text="Reset", command=self.reset_data, width=7, font=(
            "verdana", 12, "bold"), fg="white", bg="navyblue")
...
# Right Label Frame
        ...
        search_entry = ttk.Entry(
            search_frame, width=12, font=("verdana", 12, "bold"))
        ...
        # create table
        self.student_table = ttk.Treeview(table_frame, column=("Dep", "Year", "Batch", "Sem", "ID", "Name", "Div", "Roll-No", "Gender", "DOB",
                                          "Email", "Mob-No", "Address",   "Teacher", "Photo"), xscrollcommand=scroll_x.set, yscrollcommand=scroll_y.set)
...
# ==================Function Decleration==============================
    ...
    def add_data(self):
        if self.var_dep.get() == "Select Department" or self.var_yea.get == "Select Year" or self.var_batch.get() == "Select Batch" or self.var_semester.get() == "Select Semester" or self.va_std_id.get() == "" or self.var_std_name.get() == "" or self.var_div.get() == "" or self.var_roll.get() == "" or self.var_gender.get() == "" or self.var_dob.get() == "" or self.var_email.get() == "" or self.var_mob.get() == "" or self.var_address.get() == "" or self.var_teacher.get() == "":
        ...
        if self.var_dep.get() == "Select Department" or self.var_yea.get == "Select Year" or self.var_batch.get() == "Select Batch" or self.var_semester.get() == "Select Semester" or self.va_std_id.get() == "" or self.var_std_name.get() == "" or self.var_div.get() == "" or self.var_roll.get() == "" or self.var_gender.get() == "" or self.var_dob.get() == "" or self.var_email.get() == "" or self.var_mob.get() == "" or self.var_address.get() == "" or self.var_teacher.get() == "":
            ...
            try:
                Upadate = messagebox.askyesno(
                    "Upadate", "Do you want to Update this Student Details!", parent=self.root)
                if Upadate > 0:
                    conn = mysql.connector.connect(
                        host='localhost', username='root', password='root', database='face_recognition')
                    mycursor = conn.cursor()
                    mycursor.execute("update student set Dep=%s,Year=%s,Batch=%s,Semester=%s,Name=%s,Division=%s,Roll=%s,Gender=%s,DOB=%s,Email=%s,Phone=%s,Address=%s,Teacher=%s,PhotoSample=%s where Student_id=%s", (
                    self.var_dep.get(),
                    self.var_yea.get(),
                    self.var_batch.get(),
                    self.var_semester.get(),
    ...
    def search_data(self):
        if self.var_search.get() == "" or self.var_searchTX.get() == "Select":
            ...
            try:
                ...
                sql = "SELECT Student_id,Name,Dep,Batch,Year,Semester,Division,Gender,DOB,Mobile_No,Address,Roll_No,Email,Teacher,PhotoSample FROM student where Roll_No='" + \
